...
class WorkspaceNameModel(object):
    ...
    def add_listener(self, listener):
        if not isinstance(listener, WorkspaceNameModel.IListener):
            raise ValueError("WorkspaceNameModel: the specified listener is not of type IListener")
        if listener in self._listeners:
            raise ValueError("WorkspaceNameModel: the specified listener is already registered")

        self._listeners.append(listener)

    def remove_listener(self, listener):
        if listener not in self._listeners:
            raise ValueError("WorkspaceNameModel: the specified listener is not registered")
    ...
    def _validate_filename(value):
        if not isinstance(value, str):
            raise ValueError("WorkspaceNameModel: the specified file \"{0}\" is invalid.".format(value))
    ...
    def factor(self, value):
        if not isinstance(value, float):
            raise ValueError("WorkspaceNameModel: the specified value \"{0}\" is not a float.".format(value))
    ...
    def debug_mode(self, value):
        if not isinstance(value, bool):
            raise ValueError("WorkspaceNameModel: the specified value \"{0}\" is not a boolean.".format(value))
